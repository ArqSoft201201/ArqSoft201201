...
$(document).ready(function(){
    ...
    $("#salvar-form").click(function(){
        ...
        sala        = $("#numsal").val();
        predio      = $("#numpre").val();
        informacoes = $("#desatrirec").val();
        tipo        = $(".tipo:radio:checked").val();
        status      = $(".status:radio:checked").val();
        $.ajax({
            type: "POST",
            url: "http://localhost/sgri/recurso/salvar",
            data:"codigo="+codigo+"&nome="+nome+"&marca="+marca+"&sala="+sala+"&predio="+predio+"&informacoes="+informacoes+"&tipo="+tipo+"&status="+status,
